...
export async function getImdbIdForTitle(
  ...
  if (normalizedTitle.includes('american murder')) {
    console.log('🔍 Searching for AMERICAN MURDER - Debug Mode');
  }
    ...
    for (const searchType of searchTypes) {
      ...
      for (const query of queryVariations) {
        try {
          ...
          // Debug logging for AMERICAN MURDER
          if (normalizedTitle.includes('american murder') && candidates.length > 0) {
            console.log(`📊 Found ${candidates.length} candidates for "${query}":`);
            candidates.slice(0, 5).forEach((c, i) => {
              const score = calculateCandidateScore(c, normalizedTitle);
              console.log(`  ${i + 1}. "${c.title}" (${c.date}) - Score: ${score.toFixed(1)} - Popularity: ${c.popularity?.toFixed(1)}`);
            });
          }
          ...
          if (bestCandidate) {
            // Debug logging for AMERICAN MURDER
            if (normalizedTitle.includes('american murder')) {
              const finalScore = calculateCandidateScore(bestCandidate, normalizedTitle);
              console.log(`✅ Selected: "${bestCandidate.title}" (${bestCandidate.date}) - Final Score: ${finalScore.toFixed(1)}`);
            }
            ...
            if (imdbId) {
              ...
              // Debug logging for AMERICAN MURDER
              if (normalizedTitle.includes('american murder')) {
                console.log(`🎯 IMDb ID found: ${imdbId} - Confidence: ${confidence}`);
              }
