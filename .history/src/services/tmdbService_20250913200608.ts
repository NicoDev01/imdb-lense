...
export async function getImdbIdForTitle(
  ...
  console.log('🔄 Variations:', queryVariations);
  ...
  for (const lang of languages) {
    ...
    for (const searchType of searchTypes) {
      console.log(`🎬 Trying search type: ${searchType.name}`);
      ...
      for (const query of queryVariations) {
        ...
        try {
          ...
          if (candidates.length > 0) {
            console.log('👥 Candidates:', candidates.map(c => ({ title: c.title, type: c.media_type, id: c.id })));
          }

          const bestCandidate = selectBestCandidate(candidates, normalizedTitle);

          if (bestCandidate) {
            console.log('✅ Best candidate:', bestCandidate.title, '(ID:', bestCandidate.id, ')');
            ...
            if (imdbId) {
              ...
              console.log('🎉 Success! Score:', matchScore, 'Confidence:', confidence);
              ...
              console.log('❌ No IMDb ID in details');
            }
          } else {
