...
export const MovieTitlesList = React.memo<MovieTitlesListProps>(function MovieTitlesList({
  ...
  // Only show "no matches" if we're not loading AND have processed some titles AND no data at all
  const hasTitlesButNoMatches = titles.length > 0 &&
    (!movieData || movieData.length === 0) &&
    !isLoadingImdb &&
    processedTitles.size > 0 &&
    newTitlesCount === 0 &&
    (!ratingsData || ratingsData.length === 0); // Also check ratings data
