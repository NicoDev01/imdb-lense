...
export const MovieTitlesList = React.memo<MovieTitlesListProps>(function MovieTitlesList({
  ...
  const { data: newMovieData, isLoading: isLoadingImdb, isError: hasImdbError, newTitlesCount } = useBatchImdbIds(
    titles,
    { language: 'de-DE', region: 'DE' },
    processedTitles
  );

  // Combine old and new movie data
  const movieData = useMemo(() => {
    // This would need to be passed from parent or stored in state
    // For now, just use new data
    return newMovieData || [];
  }, [newMovieData]);
