...
import { extractTextFromImage, cleanMovieTitle } from '@/services/ocrService';
  ...
  const processImage = async (imageUrl: string) => {
    try {
      ...
      if (cleanedTitles.length > 0) {
        onTitlesExtracted(cleanedTitles);
        toast({
          title: 'Erfolgreich!',
          description: `${cleanedTitles.length} Filmtitel gefunden`,
        });
      } else {
        toast({
